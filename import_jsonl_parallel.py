#!/usr/bin/env python3
"""
Parallel import script - handles multiple processes safely.
Run with: for i in {0..3}; do INPUT_PATH=recipe_chunks/chunk_00$i.jsonl python3 import_jsonl_parallel.py & done
"""
import os
import re
import json
...
def import_batch_supabase(batch: List[Dict[str, Any]]):
    """Import batch using Supabase REST API with async calls."""
    for r in batch:
        ...
        try:
            ...
            print(f"⚠️  Error upserting recipe {source_uid}: {e}", flush=True)
            continue
        
        if not recipe_id:
            print(f"⚠️  Failed to get recipe_id for {source_uid}", flush=True)
            continue
        ...
        if isinstance(steps, list) and len(steps) > 0:
            ...
            if step_values:
                try:
                    ...
                    pass
                for i in range(0, len(step_values), 100):
                    chunk = step_values[i:i+100]
                    try:
                        supabase.table("recipe_steps").insert(chunk).execute()
                    except Exception as e:
                        print(f"⚠️  Warning inserting steps: {e}", flush=True)
        ...
        if isinstance(ings, list) and len(ings) > 0:
            ...
            if ing_values:
                try:
                    ...
                    print(f"⚠️  Warning deleting ingredients: {e}", flush=True)
                
                for i in range(0, len(ing_values), 100):
                    chunk = ing_values[i:i+100]
                    try:
                        supabase.table("recipe_ingredients").insert(chunk).execute()
                    except Exception as e:
                        print(f"⚠️  Warning inserting ingredients: {e}", flush=True)
    ...
    try:
        ...
        for rec in read_jsonl(INPUT_PATH):
            ...
            if len(buffer) >= BATCH_SIZE:
                ...
                total += len(buffer)
                print(f"✅ Batch {batch_no}: {len(buffer)} recipes (total {total})", flush=True)
                sys.stdout.flush()
                buffer.clear()
