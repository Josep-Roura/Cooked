...
import { buildRationale, computeNutritionTargets } from "@/lib/nutrition/targets.js"
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.info("[AI] generate-plan start", { userId: user.id, date })
    ...
    if (existingPlan?.locked && !force) {
      return NextResponse.json(
        { ok: false, error: "Plan is locked. Use force=true to overwrite." },
        { status: 409 },
      )
    }
...
Only output JSON.`
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("[AI] openai call failed", errorBody)
        return NextResponse.json({ ok: false, error: "AI request failed", details: errorBody }, { status: 502 })
      }

      aiResult = await response.json()
    } catch (error) {
      if (error instanceof Error && error.name === "AbortError") {
        return NextResponse.json({ ok: false, error: "AI request timed out." }, { status: 504 })
      }
      throw error
    } finally {
    ...
    console.info("[AI] openai call ok", { userId: user.id, date })
    ...
    const parsed = parsedJson ? PlanSchema.safeParse(parsedJson) : { success: false }
    ...
    if (!parsed.success) {
      return NextResponse.json({ ok: false, error: "Invalid AI response" }, { status: 400 })
    }
    ...
    if (planError || !planRow) {
      return NextResponse.json({ ok: false, error: "Failed to save plan" }, { status: 400 })
    }
    ...
    for (const [index, meal] of planData.meals.entries()) {
      ...
      if (recipeError || !recipeRow) {
        return NextResponse.json({ ok: false, error: "Failed to save recipe" }, { status: 400 })
      }
      ...
      if (itemError) {
        return NextResponse.json({ ok: false, error: "Failed to save meal items" }, { status: 400 })
      }
    }
    ...
    let nutritionPlanId = existingNutritionPlan?.id ?? null
    ...
    if (!nutritionPlanId) {
      ...
      if (createPlanError || !createdPlan) {
        return NextResponse.json({ ok: false, error: "Failed to save nutrition plan" }, { status: 400 })
      }

      nutritionPlanId = createdPlan.id
    }

    const { error: rowError } = await supabase.from("nutrition_plan_rows").upsert(
      {
    ...
    if (rowError) {
      return NextResponse.json({ ok: false, error: "Failed to save nutrition plan rows" }, { status: 400 })
    }
