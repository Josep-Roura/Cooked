...
import type { Meal, NutritionDayPlan, NutritionDayType, NutritionMacros } from "@/lib/db/types"
...
export async function POST(req: NextRequest) {
  try {
    ...
    const meals = alignMealTimes(mealsWithMacros, workouts ?? [])

    if (!regenerate) {
      const { data: existingRows, error: existingError } = await supabase
        .from("nutrition_plan_rows")
        .select("plan_id, date, day_type, kcal, protein_g, carbs_g, fat_g, intra_cho_g_per_h")
        .eq("user_id", user.id)
        .eq("date", date)
      ...
      if (existingError) {
        return NextResponse.json(
          { error: "Failed to check existing plan", details: existingError.message, code: existingError.code },
          { status: 400 },
        )
      }
      ...
      if (existingRows && existingRows.length > 0) {
        ...
        const payload: NutritionDayPlan = {
          ...
          date: existing.date,
          day_type: existing.day_type === "high" || existing.day_type === "rest" ? existing.day_type : "training",
          macros: {
            kcal: existing.kcal,
            protein_g: existing.protein_g,
            carbs_g: existing.carbs_g,
            fat_g: existing.fat_g,
            intra_cho_g_per_h: existing.intra_cho_g_per_h,
          },
    ...
    const { data: plan, error: planError } = await supabase
      ...
      .insert({
        user_id: user.id,
        user_key: user.id,
        source_filename: "generated_daily_plan",
        weight_kg: weightKg,
        start_date: date,
        end_date: date,
      })
    ...
    const { data: planRow, error: rowInsertError } = await supabase
      .from("nutrition_plan_rows")
      .insert({
        plan_id: plan.id,
        date,
    ...
    if (rowInsertError || !planRow) {
      return NextResponse.json(
        { error: "Failed to save nutrition plan row", details: rowInsertError?.message ?? null },
        { status: 400 },
      )
    }
    ...
    console.error("POST /api/v1/nutrition/generate error:", error)
    return NextResponse.json(
